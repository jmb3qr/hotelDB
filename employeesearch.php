...
$_SESSION["error"] = "You have not logged in. Please log in first";
 ...
 echo "Failed to connect to MySQL: " . mysqli_connect_error();
        ...
        if($stmt->prepare("select Reservation_ID, Last_Name, First_Name, Email, Phone_Number, Room_number, Start_Date, End_Date, Hotel_Name from Reservation natural join MAKES natural join Guest natural join ASSIGNED natural join Rooms natural join Hotel where Last_Name like ? or First_Name like ? or Email like ? or Start_Date like ? or End_Date like ?") or die(mysqli_error($db))) {
                $searchString = '%' . $_GET['GuestName'] . '%';
                $stmt->bind_param('sssss', $searchString, $searchString, $searchString, $searchString, $searchString);
                $stmt->execute();
                $stmt->bind_result($id, $lname, $fname, $email, $phone, $num, $start, $end, $hotel);
